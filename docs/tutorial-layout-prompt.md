...
14a. CSS requirement: Canonical `.code-panel__line` elements must have `white-space: normal;` to collapse formatting whitespace and maintain tight line spacing. Never use `white-space: pre;` on code-panel lines. Legacy `.pseudo-panel__line` aliases exist only for compatibility.
